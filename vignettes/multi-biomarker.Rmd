---
title: "Biomarker Comparison with seroCOP"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Biomarker Comparison with seroCOP}
...
This vignette demonstrates how to compare independent univariate models for multiple biomarkers using the `SeroCOPCompare` class. We'll simulate three biomarkers with different correlates of protection:
...
# Simulate Biomarker Comparison Data
...
# Compare Independent Biomarker Models

Now we'll use the `SeroCOPCompare` class to fit a separate univariate model for each biomarker:
...
```{r individual-roc, fig.height=4, fig.width=10}
# Plot ROC curves for each biomarker using ggplot2
roc_plot_list <- lapply(multi_model$biomarker_names, function(bm) {
  m       <- multi_model$models[[bm]]
  pred    <- colMeans(m$predict())
  roc_obj <- pROC::roc(m$infected, pred, quiet = TRUE)
  auc_val <- as.numeric(pROC::auc(roc_obj))
  data.frame(
    biomarker    = sprintf("%s (AUC = %.3f)", bm, auc_val),
...
The `SeroCOPCompare` class enables comparison of independent biomarker models to identify the strongest univariate correlates of protection.
